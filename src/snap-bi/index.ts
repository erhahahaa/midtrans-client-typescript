...
import type { SnapBiConfig } from "./config";
...
export class SnapBi {
	...
	async createPayment(externalId: string): Promise<PaymentResponse> {
		...
		return (await this.createConnection(externalId)) as PaymentResponse;
	}
	...
	async isWebhookNotificationVerified(): Promise<boolean> {
		...
		const minifiedNotificationBodyJsonString = JSON.stringify(
	...
	async getAccessToken(): Promise<AccessTokenResponse> {
		const snapBiAccessTokenHeader = await this.buildAccessTokenHeader(
			this.timeStamp,
		);
	 ...
	 */
	private async createConnection(externalId: string): Promise<unknown> {
		if (!this.accessToken) {
	 ...
	 */
	private async buildAccessTokenHeader(
		timeStamp: string,
	): Promise<RequestHeaders> {
	...
	private static async hmacSha512(
		data: string,
		secret: string,
	): Promise<string> {
		const encoder = new TextEncoder();
	...
	private static async signWithRsa(
		data: string,
