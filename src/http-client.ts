 ...
 */
export type RequestParams = Record<string, unknown> | string;

/**
 * Midtrans API response
 */
export interface MidtransApiResponse extends Record<string, unknown> {
...
export class HttpClient {
	...
	async request<T = MidtransApiResponse>(
		...
		if (
			...
			Object.keys(reqBodyPayload).length > 0
		) {
